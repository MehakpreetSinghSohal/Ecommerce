...
const Home = () => {
  ...
  const offerProducts = productData.filter((product) =>
    product.tagline.toLowerCase().includes("offer")
  );

  const trendingProducts = productData.filter(
    (product) => product.price.discountPercent >= 20
  );
      ...
      {showNetworkError ? (
        ...
        <>
          {responseProducts ? (
            ...
            <>
              <Slide products={trendingProducts.slice(0, 4)} title="Trending" />

              <Slide products={offerProducts.slice(0, 4)} title="Offers" />
